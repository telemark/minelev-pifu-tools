...
const teacherBasisClasses = []
const teacherContactClasses = []
teacherClassIds.forEach(id => {
  const classGroup = groups.find(group => group.sourcedid.id === id)
  if (classGroup) {
    if (classGroup.grouptype.typevalue.text === 'basisgruppe') {
      teacherBasisClasses.push({
        id,
    ...
    if (classGroup.grouptype.typevalue.text === 'kontaktlærergruppe') {
      teacherContactClasses.push({
        id,
        type: classGroup ? classGroup.grouptype.typevalue.text : undefined,
        name: classGroup ? classGroup.description.short : undefined,
        school: classGroup ? classGroup.relationship?.label : undefined
...
teacherObj.basisClasses = teacherBasisClasses
teacherObj.contactClasses = teacherContactClasses

const lastModified = statSync(resolve('data/memberships.json')).mtime
console.log(`This is data from ${lastModified}\n\nTeacher is basis teacher for ${teacherBasisClasses.length} classes and contact teacher for ${teacherContactClasses.length} classes :`, teacherObj)
