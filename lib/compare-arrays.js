...
module.exports = (old = [], fresh = []) => {
  ...
  fresh.forEach(freshObj => {
    const usernameOrId = ['student', 'teacher'].includes(freshObj.type) ? freshObj.username || 'unknownUnsern' : freshObj.id
  ...
  logger('info', ['compare-arrays', 'return', 'updates', updates.length])
