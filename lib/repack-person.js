...
module.exports = person => {
  ...
  const { fagIds, programomraadeIds, utdanningsprogramIds } = groupIds
  const groups = type === 'student' ? faggrupper.filter(group => fagIds.includes(group.id)).map(cleanUpGroup) : []
  const userProgramomraader = type === 'student' ? programomraader.filter(group => programomraadeIds.includes(group.id)).map(group => group.grep) : []
  const userUtdanningsprogrammer = type === 'student' ? utdanningsprogrammer.filter(group => utdanningsprogramIds.includes(group.id)).map(group => group.grep) : []
